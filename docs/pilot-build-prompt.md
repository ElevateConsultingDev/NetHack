...
**When the brain is asked matters more than what it knows.** Consulting the model at checkpoints (new level, first sight of a monster type, every 500 turns) cost half a level of XL and gained no depth against the same model answering escalations only; at escalations alone it tied the fixed rules exactly (43 of 64 held-out games identical). Start with the brain answering escalations only, and prove any added consult on held-out seeds.
