...
| Reproducible games | `NETHACK_SEED` fixes the game's RNG (and stops 3.6's per-level reseeding), `NETHACK_NOW` fixes its clock (moon, night, Friday 13th). `python3 -m pilot.batch --seed N` gives game i seed N+i, no bones, and records every key and brain answer; `--replay RUN/NAME` reruns one with the recorded answers and reports whether every key matched or where it first diverged. Same `--seed` for two batches = the same dungeons (paired comparison). |
