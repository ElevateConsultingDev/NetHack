...
7. **Every third iteration, two checks** (log the result under "Check log" below):
   ...
   - **Held-out seeds.** `--brain rules --seed 5000 --games 32`, compared with the previous held-out run. Keep or drop never looks at these seeds, so they show whether gains made on seed 1000 carry over to dungeons the loop hasn't tuned for. If seed 1000 improved and 5000 didn't, say so in the report.
