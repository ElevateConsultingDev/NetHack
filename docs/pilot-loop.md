...
1. **Baseline.** Use the latest Haiku batch `playground/batch/<run>.json` as the baseline (run `python3 -m pilot.batch --games 16 --parallel 4 --brain haiku` if there is none since the last engine change).
...
4. **Check.** `python3 -m pilot.test_prayer` and any other self-checks; replay the saved scenarios in that bucket (`python3 -m pilot.scenario list`, `run <name>`).
5. **Measure.** Run a new 16-game Haiku batch.
